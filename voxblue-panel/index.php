...
<head>
    ...
    <style>
        ...
        .dot.free {
            ...
            box-shadow: 0 0 5px var(--st-free);
        }

        .dot.busy {
            background: var(--st-busy);
            box-shadow: 0 0 5px var(--st-busy);
        }
        ...
        .mini-card.free::before {
            background: var(--st-free);
            box-shadow: 2px 0 8px -2px rgba(34, 197, 94, 0.4);
        }

        .mini-card.busy::before {
            background: var(--st-busy);
            box-shadow: 2px 0 8px -2px rgba(239, 68, 68, 0.4);
        }

        .mini-card.ring::before {
            background: var(--st-ring);
...
<body>
    ...
    <nav class="top-nav">
        ...
        <div class="nav-actions">
            ...
            <a href="../" title="Sair"><i class="bi bi-power"></i></a>
        </div>
    </nav>
    ...
    <div class="monitor-bar">
        ...
        <div class="legend">
            <div class="l-item">
                <div class="dot free"></div> Livre
            </div>
            <div class="l-item">
                <div class="dot busy"></div> Em Uso
            </div>
            <div class="l-item">
                <div class="dot ring"></div> Chamando
            </div>
            <div class="l-item">
    ...
    <script>
        ...
        function renderFilas(filas) {
            ...
            if (!filas || filas.length === 0) {
                el.innerHTML = '<span class="text-muted small">Sem filas.</span>';
                return;
            }
        ...
        function renderRamais(ramais) {
            ...
            if (filtered.length === 0) {
                el.innerHTML = '<div class="text-muted small w-100 text-center py-5">Nenhum ramal encontrado.</div>';
                return;
            }
            ...
            el.innerHTML = filtered.map(r => {
                let vClass = 'off';
                let icon = 'bi-dash-lg';
                if (r.css_class === 'st-free') {
                    vClass = 'free';
                    icon = 'bi-check-lg';
                }
                if (r.css_class === 'st-busy') {
                    vClass = 'busy';
                    icon = 'bi-telephone-fill';
                }
                if (r.status === 'CHAMANDO') {
                    vClass = 'ring';
                    icon = 'bi-bell-fill';
                }

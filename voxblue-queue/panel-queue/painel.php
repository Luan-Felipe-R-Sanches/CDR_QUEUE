...
<head>
    ...
    <style>
        ...
        .agent-card {
            ...
            overflow: hidden;
            min-height: 150px;
            /* Altura aumentada para botões */
            display: flex;
            flex-direction: column;
            justify-content: center;
            align-items: center;
            transition: all 0.3s ease;
            /* Animação suave na atualização */
        }
...
<body>
    ...
    <div class="legend-bar">
        <div class="legend-item">
            <div class="legend-dot bg-success"></div> LIVRE
        </div>
        <div class="legend-item">
            <div class="legend-dot bg-danger"></div> FALANDO
        </div>
        <div class="legend-item">
            <div class="legend-dot bg-warning"></div> PAUSA
        </div>
        <div class="legend-item">
            <div class="legend-dot bg-secondary"></div> OFF
        </div>
    </div>
    ...
    <script>
        ...
        $('#myExt').on('change', function() {
            localStorage.setItem('sup_ext', $(this).val());
        });
        ...
        function cmdSpy(target, mode) {
            let sup = $('#myExt').val();
            if (!sup) {
                showToast('Defina seu ramal!', 'warning');
                $('#myExt').focus();
                return;
            }
            $.post('api_control.php', {
                ...
                target: target,
                mode: mode
            }, d => showToast(d.status, 'primary'), 'json');
        }
        ...
        function openAdd(qid, name) {
            $('#mdQueueId').val(qid);
            $('#mdQueueName').text(name);
            addModal.show();
        }

        window.doAddMember = () => {
            let qid = $('#mdQueueId').val(),
                iface = $('#mdUserSelect').val();
            if (!iface) {
